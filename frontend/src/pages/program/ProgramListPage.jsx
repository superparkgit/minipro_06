...
function ProgramListPage() {
  ...
  useEffect(() => {
    getPrograms()
      ...
      .catch((requestError) => setError(getApiErrorMessage(requestError, '프로그램 목록을 불러오지 못했습니다.')))
  ...
  return (
    <>
      <section className="section-heading">
        ...
        <Link className="button button-secondary" to="/programs/new">트레이너 등록</Link>
      </section>
      {loading && <p className="notice">프로그램을 불러오는 중입니다.</p>}
      {error && <p className="notice notice-error">{error}</p>}
      {!loading && !error && programs.length === 0 && <p className="page-card">등록된 프로그램이 없습니다.</p>}
