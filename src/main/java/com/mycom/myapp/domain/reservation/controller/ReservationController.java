...
public class ReservationController {
    ...
    @GetMapping("/programs/{id}/reservations")
    public ResponseEntity<List<ReservationResponse>> getReservationsByProgram(
            @PathVariable Long id,
            @AuthenticationPrincipal CustomUserDetails userDetails) {
        return ResponseEntity.ok(reservationService.getReservationsByProgram(id, userDetails.getUserId()));
    }
    ...
    @PatchMapping("/reservations/{id}/attendance")
    public ResponseEntity<ReservationResponse> markAttendance(
            @PathVariable Long id,
            @Valid @RequestBody AttendanceRequest request,
            @AuthenticationPrincipal CustomUserDetails userDetails) {
        return ResponseEntity.ok(reservationService.markAttendance(id, userDetails.getUserId(), request));
    }
}
